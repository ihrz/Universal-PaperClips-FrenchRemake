...
function addProc() {
  if (trust > 0 || swarmGifts > 0) {
    ...
    if (creativityOn == 1) {
      displayMessage(
        "Processeur ajouté, opérations (ou créativité) par secondes augmenté"
      );
    } else {
      displayMessage("Processeur ajouté, opérations par secondes augmenté");
    }
...
function addMem() {
  if (trust > 0 || swarmGifts > 0) {
    displayMessage(
      "Mémoire vive ajouté, le maximum d'opérations possible à été augmenté."
    );
    memory = memory + 1;
...
function milestoneCheck() {
  ...
  if (milestoneFlag == 8 && Math.ceil(clips) >= 1000000000000000) {
    milestoneFlag = milestoneFlag + 1;
    displayMessage(
      "Un Quadrilliard de trombones crée en " + timeCruncher(ticks)
    );
  }
  ...
  if (milestoneFlag == 9 && Math.ceil(clips) >= 1000000000000000000) {
    milestoneFlag = milestoneFlag + 1;
    displayMessage(
      "Un Quintilliard de trombones crée en  " + timeCruncher(ticks)
    );
  }
